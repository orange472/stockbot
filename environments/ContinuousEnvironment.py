...
class ContinuousSingleStockEnvironment(gym.Env):
    ...
    def __init__(
        ...
        super(ContinuousSingleStockEnvironment, self).__init__()
        ...
        # Max steps is equal to length of data
    ...
    def step(self, action: float) -> Tuple[np.ndarray, float, bool, Dict[str, Any]]:
        ...
        if action < 0.3 and action > -0.3:
            ...
            self.portfolio["net"] += sale - transaction_fee
            self.balance += sale
        ...
        elif action >= 0.3 and self.balance / curr_price < 1:
            ...
            self.transaction["num_shares"] = max(
                1, math.floor(action * self.balance / curr_price)
            )
            ...
            self.portfolio["num_shares"] += self.transaction["num_shares"]
            self.portfolio["net"] -= cost + transaction_fee
        ...
        if self.transaction["type"] != "HOLD" and self.portfolio["num_shares_sold"] > 0:
            self.rewards["buy_sell_diff"] = self._pct_change(
